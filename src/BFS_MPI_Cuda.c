							...
							unsigned int* final_node)
{
  ...
  if (vertex < *graph_num_vertices) {
	if (level[vertex] == *currLevel) {
	  ...
	  if (vertex == *final_node) *found = 1;
	  for (unsigned int edge = graph_row_ptrs[vertex - graph_row_ptrs[0]];
		   edge < graph_row_ptrs[vertex - graph_row_ptrs[0] + 1];
		   ++edge) {
...
int mpi_cuda_execute_bfs(CSR_Graph* graph, int final_node) {
  printf("Hello from the beginning of execute_bfs\n");
	...
	while (host_newVertexVisited && !host_found) {
      ...
      // Call the BFS kernel
	  bfs_kernel <<<numBlocks, blockSize>>>(device_graph_row_ptrs, device_graph_dst, device_level, device_newVertexVisited, device_found, device_currLevel, device_graph_num_vertices, device_final_node);
